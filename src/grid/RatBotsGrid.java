...
public class RatBotsGrid<E> extends AbstractGrid<E>
{
    ...
    public Location getRandomEmptyTreeLocation()
    {
        Location loc;
        ...
        do
        {
            ...
            if(isNearLake(loc)) endLoop = false;
    ...
    public boolean isNearLake(Location loc)
    {
        int LAKE_PROXIMITY = 4;

        ArrayList<Location> allLakeLocations = new ArrayList<Location>();
        for(int r=0; r<18; r++)
            for(int c=0; c<18; c++)
                if(occupantArray[r][c] instanceof Lake)
                    allLakeLocations.add(new Location(r,c));
        for(Location lakeLoc : allLakeLocations)
        {
            if(loc.distanceTo(lakeLoc) < LAKE_PROXIMITY)
                return true;
        }
